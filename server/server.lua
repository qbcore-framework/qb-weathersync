...
RegisterCommand('freezeweather', function(source, _, _)
    if isAllowedToChange(source) then
...
RegisterCommand('blackout', function(source, _, _)
    local src = source
    if isAllowedToChange(src) then
...
RegisterCommand('morning', function(source, _, _)
    if isAllowedToChange(source) then
        setTime(9, 0)
...
RegisterCommand('noon', function(source, _, _)
    if isAllowedToChange(source) then
        setTime(12, 0)
...
RegisterCommand('evening', function(source, _, _)
    if isAllowedToChange(source) then
        setTime(18, 0)
...
RegisterCommand('night', function(source, _, _)
    if isAllowedToChange(source) then
        setTime(23, 0)
